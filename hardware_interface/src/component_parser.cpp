...
constexpr const auto kManageOverrunsAttribute = "manage_overruns";
...
void auto_fill_transmission_interfaces(HardwareInfo & hardware)
{
  for (auto & transmission : hardware.transmissions)
  {
    ...
    if (hardware.type == kActuatorTag)
    {
      ...
      {
        throw std::runtime_error(fmt::format(
          FMT_COMPILE("Error while parsing '{}'. There should be exactly one joint defined in this "
                      "component but found {}."),
    ...
    else if (std::string(kPropertiesTag) == ros2_control_child_it->Name())
    {
      ...
      if (async_it)
      {
        ...
        try
        {
          ...
          if (async_it->FindAttribute(kManageOverrunsAttribute))
          {
            hardware.async_params.manage_overruns =
              parse_bool(get_attribute_value(async_it, kManageOverrunsAttribute, kAsyncTag));
          }
        }
        catch (const std::exception & e)
        {
