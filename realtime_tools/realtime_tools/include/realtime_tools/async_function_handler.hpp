 ...
 * @param manage_overruns Whether to manage overruns by skipping missed cycles. If true, when the callback
 * execution takes longer than the desired period, the next iteration time will be adjusted to skip missed cycles.
 */
struct AsyncFunctionHandlerParams
{
  ...
  void initialize(NodeT & node, const std::string & prefix)
  {
    ...
    if (node->has_parameter(prefix + "overruns.manage")) {
      manage_overruns = node->get_parameter(prefix + "overruns.manage").as_bool();
    }
    RCLCPP_INFO(
      logger, "AsyncFunctionHandler: manage_overruns is set to %s",
      manage_overruns ? "true" : "false");
  }
            ...
            if (next_iteration_time < time_now) {
              ...
              if (params_.print_warnings) {
                RCLCPP_WARN_THROTTLE(
                  ...
                  "Overrun detected! The async callback missed its desired rate of %u Hz. The loop "
